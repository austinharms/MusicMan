...
import { FFmpeg, FFmpegOptions, opus } from "prism-media";
...
export class SongStream {
    ...
    public async init(): Promise<void> {
        ...
        const ffmpegArgs: string[] = ["-i", this._song.playbackURL?.href as string, ...BASE_FFMPEG_ARGS];
        ...
        // Create Opus Stream
        this._opusStream = new opus.Encoder({ rate: 48000, channels: 2, frameSize: 960 });
        this._opusStream.once("error", this.boundError);

        // Create discordjs Resource
        this._resource = createAudioResource(
            this._opusStream,
            {
    ...
    public destroy(): void {
        ...
        if (this._resource?.audioPlayer)
            this._resource.audioPlayer.stop();
